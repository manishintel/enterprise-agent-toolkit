...
/**
 * Get the current username from the NextAuth session.
 * Sent to the backend as `X-Forwarded-User` so jobs/files can be attributed.
 */
export async function getNextAuthUsername(): Promise<string | null> {
  if (typeof window === 'undefined') return null;
  const session = await getSession();
  return session?.user?.name || (session?.user as { id?: string } | undefined)?.id || null;
}

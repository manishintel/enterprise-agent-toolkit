...
const NewFineTuningJobPage = () => {
  ...
  // Default the model name from the selected base model, so the deployed model
  // is identifiable without having to look the job up. Anything typed by hand
  // wins from then on.
  const handleValuesChange = (changed: Partial<FineTuningFormValues>) => {
    if ('suffix' in changed) {
      setModelNameEdited(Boolean(changed.suffix));
      return;
    }
    if ('model' in changed && changed.model && !modelNameEdited) {
      form.setFieldsValue({ suffix: buildFineTunedModelName(changed.model, new Date()) });
    }
  };
      ...
      <Card>
        ...
        >
          ...
          <Form.Item
            ...
            extra="The name the fine-tuned model is served and registered under. Defaults to the base model plus a timestamp; change it if you want something else."
          >
            <Input
              placeholder="Select a base model to generate a name"

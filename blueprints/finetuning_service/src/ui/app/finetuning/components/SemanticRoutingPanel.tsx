 ...
 * the deployment page. The work is a three-step review that a user iterates on --
 * extract, prune, test, adjust, test again -- which a dialog is a poor container
 * for: it steals the page, cannot be linked to, and encourages a "just press
 * Apply" reading of what is really a calibration task.
 *
    ...
    <Space orientation="vertical" style={{ width: '100%' }} size="middle">
        ...
        <Descriptions size="small" column={1} bordered>
          <Descriptions.Item label="Clients call">
            ...
            <Text type="secondary" style={{ fontSize: 11 }}>
              — routing only happens for requests addressed to this name; traffic sent to a specific
              model is unaffected.
            </Text>
          </Descriptions.Item>
        ...
        <Alert
          ...
          description={
            <Space orientation="vertical" size={2} style={{ fontSize: 12 }}>
              ...
              {redacted.length > 0 && (
                <span>Redacted: {redacted.map(([kind, count]) => `${count}× ${kind}`).join(', ')}</span>
              )}
      ...
      <div>
        ...
        <Paragraph type="secondary" style={{ fontSize: 11, marginBottom: 8 }}>
          ...
          utterances — not its best match — so the number is lower than it looks. On a banking set,
          in-domain queries scored 0.57-0.62 and unrelated ones 0.40-0.44. Test a couple of real
      ...
      {status?.restart_required_on_apply && (
        <Alert
          ...
          description={
            <>
              The gateway caches a router in memory, so a change only takes effect once it restarts.
              Requests in flight during the restart may fail; it is usually back within a minute. Job{' '}
              {jobId} is unaffected.
            </>
          }
        />
      ...
      {/* Actions sit at the bottom of the flow rather than in a dialog footer, so
          they read as the end of the three steps above. */}
      <Card size="small" style={{ background: '#fafafa' }}>
        <Space wrap>
          <Button
            ...
            loading={applying}
            disabled={!canApply}
            onClick={() => onApply(utterances, threshold)}
          >
            {applied ? 'Update route' : 'Apply route'}
          </Button>
          {applied && (
            <Button danger icon={<DeleteOutlined />} loading={removing} onClick={onRemove}>
              Stop routing here
            </Button>
          )}
          {!applied && utterances.length === 0 && (
            <Text type="secondary" style={{ fontSize: 12 }}>
              Extract or add at least one utterance to apply a route.
            </Text>
          )}
        </Space>
      </Card>

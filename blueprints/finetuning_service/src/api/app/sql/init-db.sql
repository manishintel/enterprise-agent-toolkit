...
CREATE TABLE IF NOT EXISTS fine_tuning_jobs (
    ...
    updated_at INTEGER,
    started_at INTEGER,   -- when the training engine picked the job up (queue wait ends)
    finished_at INTEGER,
    ...
    -- Live training progress (refreshed from the backend engine on each status poll)
...
-- Backfill progress columns on databases created before they were added, so
-- re-running this script upgrades an existing schema instead of failing later
-- when the resource adapter writes live training progress.
ALTER TABLE fine_tuning_jobs ADD COLUMN IF NOT EXISTS progress_percent REAL;

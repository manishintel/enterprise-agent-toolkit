...
class TrainingJob(Base):
    ...
    num_train_epochs = Column(Float, nullable=True)
    # Which stage of the pipeline the job is in. `status` stays coarse
    # (PENDING/RUNNING/...) so existing consumers are unaffected; this is the
    # detail a progress bar needs while `status` is RUNNING. Values are the
    # PHASE_* constants in app/services/progress.py.

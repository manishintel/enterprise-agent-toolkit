...
def _load_allowlist() -> set:
    """
    Resolve the base-model allowlist.

    Deployments differ in which models they can actually pull (gated repos,
    offline caches), so the list is overridable through the
    ALLOWED_BASE_MODELS setting. Unset means the defaults above apply, i.e. the
    policy is unchanged unless an operator deliberately edits it.
    """
    try:
        from app.config import settings  # noqa: PLC0415 - optional dependency here

        raw = settings.ALLOWED_BASE_MODELS or ""
    except Exception:  # pragma: no cover - validator used without app config
        raw = ""

    configured = {model.strip() for model in raw.split(",") if model.strip()}
    if configured:
        logger.info(f"[ML-SECURITY] Base-model allowlist from config: {sorted(configured)}")
        return configured
    return set(DEFAULT_ALLOWED_BASE_MODELS)


ALLOWED_BASE_MODELS = _load_allowlist()
